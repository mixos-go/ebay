---
title: bulkUpdateKeyword
category: Marketing_API
api_name: bulkUpdateKeyword
method: POST
path: /ad_campaign/{campaign_id}/bulk_update_keyword
---

**Category:** Marketing_API
**API:** bulkUpdateKeyword

**Method:** POST
**HTTP Path:** https://api.ebay.com{basePath}/ad_campaign/{campaign_id}/bulk_update_keyword
...
| --- | --- | --- | --- |
| campaign_id (path) | string | Yes | This path parameter specifies the unique eBay-assigned identifier of the ad campaign for which a set of keywords is being updated. Use the getCampaigns method to retrieve campaign IDs. |
| Content-Type (header) | string | Yes | This header indicates the format of the request body provided by the client. Its value should be set to application/json . For more information, refer to HTTP request headers . |

## Request Body
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| requests | array<UpdateKeywordByKeywordIdRequest> | No | Use this array to update the bid values and/or statuses of one or more existing keywords. |
| requests.bid | Amount | No | This container is used to set or change the bid percentage for the keyword. Each time a listing is retrieved in search results using this keyword and clicked on, the seller will be charged this amount. Each click goes toward the daily budget set up for the Cost Per Click (CPC) campaign. |
...
| requests.keywordId | string | No | This field is used to identify the keyword to be updated. The getKeyword method can be used to retrieve keywordId values. |
| requests.keywordStatus | string | No | Include this field if you wish to change the status of the keyword. The status value specified here must be different than the keyword's current status. To confirm the current status of a keyword, you can use the getKeyword method. If the status of the ad is currently ACTIVE , you can change status  |

## Response
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| responses | array<UpdateKeywordResponse> | No | A list of keywords that have been processed from the bulk request. |
| responses.errors | array<Error> | No | This container will be returned if there are one or more issues associated with modifying the corresponding keyword. |
...
| responses.keywordId | string | No | This field identifies the keyword that the seller updated, or attempted to update. |
| responses.statusCode | integer | No | An HTTP status code is returned for each keyword to indicate the success or failure of updating that keyword. |
