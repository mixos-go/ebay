---
title: Contest_Payment_Dispute
category: Fulfillment_API
api_name: Contest_Payment_Dispute
method: POST
path: /payment_dispute/{payment_dispute_id}/contest
---

**Category:** Fulfillment_API
**API:** Contest_Payment_Dispute

**Method:** POST
**HTTP Path:** https://api.ebay.com{basePath}/payment_dispute/{payment_dispute_id}/contest
...
| --- | --- | --- | --- |
| payment_dispute_id (path) | string | Yes | This parameter is used to specify the unique identifier of the payment dispute being contested. Use the getPaymentDisputeSummaries method to retrieve payment dispute IDs. |
| Content-Type (header) | string | Yes | This header indicates the format of the request body provided by the client. Its value should be set to application/json . For more information, refer to HTTP request headers . |

## Request Body
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| note | string | No | This field shows information that the seller provides about the dispute, such as the basis for the dispute, any relevant evidence, tracking numbers, and so forth. Max Length: 1000 characters. |
| returnAddress | ReturnAddress | No | This container is needed if the seller is requesting that the buyer return the item. If this container is used, all relevant fields must be included, including fullName and primaryPhone . Note: If the Dispute Reason is SIGNIFICANTLY_NOT_AS_DESCRIBED , returnAddress is required. |
